...
class NM_Search_Suggestions {
        ...
        if ( ! $this->transient_enabled || false === ( $suggestions = get_transient( $transient_name ) ) ) {
            ...
            if ( ! empty( $products ) ) {
                ...
                foreach ( $products as $post ) {
                    ...
                    $products_html .= '<li><a href="' .  esc_url( $product->get_permalink() ) . '">' . $product->get_image() . '<h3>' . $product->get_title() . '</h3><span class="price">' . $product->get_price_html() . '</span></a></li>';
                }
	...
	public function instant_suggestions_get_product_data() {
        $generate_cache = intval( get_option( 'nm_instant_suggestions_generate_cache', 1 ) );
        ...
        if ( $generate_cache == 1 ) {
            ...
            $query = new WC_Product_Query( apply_filters( 'nm_instant_suggestions_query_args', array(
                'status'        => 'publish',
                'visibility'    => 'search',
                'limit'         => -1
            ) ) );
            ...
            $image_size = apply_filters( 'nm_search_suggestions_image_size', 'woocommerce_thumbnail' );
            ...
            foreach( $products as $product ) {
                ...
                $product_html = '<li><a href="' .  esc_url( $product->get_permalink() ) . '"><img src="' . $image[0] . '" width="' . $image[1] . '" height="' . $image[2] . '"><h3>' . $product->get_title() . '</h3><span class="price">' . $product->get_price_html() . '</span></a></li>';
                
                $products_array[$id] = array(
