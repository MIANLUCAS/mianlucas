...
if ( ! function_exists( 'woocustomizer_library_get_font_choices' ) ) :
/**
 ...
 */
function woocustomizer_library_get_all_fonts() {
	$heading1       = array( 1 => array( 'label' => sprintf( '--- %s ---', __( 'Standard Fonts', 'overlay' ) ) ) );
	$standard_fonts = woocustomizer_library_get_standard_fonts();
	$heading2       = array( 2 => array( 'label' => sprintf( '--- %s ---', __( 'Google Fonts', 'overlay' ) ) ) );
	$google_fonts   = woocustomizer_library_get_google_fonts();
	 ...
	 */
	return apply_filters( 'woocustomizer_library_all_fonts', array_merge( $heading1, $standard_fonts, $heading2, $google_fonts ) );
}
endif;
 ...
 * @return array    The fonts in value/label pairs.
 */
function woocustomizer_library_get_font_choices() {
	$fonts   = woocustomizer_library_get_all_fonts();
	$choices = array();
...
if ( ! function_exists( 'woocustomizer_library_get_google_font_uri' ) ) :
/**
 ...
 */
function woocustomizer_library_get_google_font_uri( $fonts ) {

	// De-dupe the fonts
	$fonts         = array_unique( $fonts );
	$allowed_fonts = woocustomizer_library_get_google_fonts();
	$family        = array();
	...
	foreach ( $fonts as $font ) {
		...
		if ( array_key_exists( $font, $allowed_fonts ) ) {
			...
			$family[] = urlencode( $font . ':' . join( ',', woocustomizer_library_choose_google_font_variants( $font, $allowed_fonts[ $font ]['variants'] ) ) );
		}
	}
	...
	$subset = get_theme_mod( 'font-subset', woowoocustomizer_library_get_default( 'font-subset' ) );

	if ( 'all' === $subset ) {
		$subsets_available = woocustomizer_library_get_google_font_subsets();
...
if ( ! function_exists( 'woocustomizer_library_get_google_font_subsets' ) ) :
/**
 ...
 */
function woocustomizer_library_get_google_font_subsets() {
	return array(
		'all'          => __( 'All', 'overlay' ),
		'cyrillic'     => __( 'Cyrillic', 'overlay' ),
		'cyrillic-ext' => __( 'Cyrillic Extended', 'overlay' ),
		'devanagari'   => __( 'Devanagari', 'overlay' ),
		'greek'        => __( 'Greek', 'overlay' ),
		'greek-ext'    => __( 'Greek Extended', 'overlay' ),
		'khmer'        => __( 'Khmer', 'overlay' ),
		'latin'        => __( 'Latin', 'overlay' ),
		'latin-ext'    => __( 'Latin Extended', 'overlay' ),
		'vietnamese'   => __( 'Vietnamese', 'overlay' ),
	);
}
endif;

if ( ! function_exists( 'woocustomizer_library_choose_google_font_variants' ) ) :
/**
 ...
 */
function woocustomizer_library_choose_google_font_variants( $font, $variants = array() ) {
	$chosen_variants = array();
	if ( empty( $variants ) ) {
		$fonts = woocustomizer_library_get_google_fonts();
	...
	return apply_filters( 'woocustomizer_library_font_variants', array_unique( $chosen_variants ), $font, $variants );
}
endif;

if ( ! function_exists( 'woocustomizer_library_get_standard_fonts' ) ) :
/**
 ...
 */
function woocustomizer_library_get_standard_fonts() {
	return array(
		'serif' => array(
			'label' => _x( 'Serif', 'font style', 'overlay' ),
			'stack' => 'Georgia,Times,"Times New Roman",serif'
		),
		'sans-serif' => array(
			'label' => _x( 'Sans Serif', 'font style', 'overlay' ),
			'stack' => '"Helvetica Neue",Helvetica,Arial,sans-serif'
		),
		'monospace' => array(
			'label' => _x( 'Monospaced', 'font style', 'overlay' ),
...
if ( ! function_exists( 'woocustomizer_library_get_font_stack' ) ) :
/**
 ...
 */
function woocustomizer_library_get_font_stack( $font ) {

	$all_fonts = woocustomizer_library_get_all_fonts();

	// Sanitize font choice
	$font = woocustomizer_library_sanitize_font_choice( $font );
...
if ( ! function_exists( 'woocustomizer_library_sanitize_font_choice' ) ) :
/**
 ...
 */
function woocustomizer_library_sanitize_font_choice( $value ) {
	if ( is_int( $value ) ) {
		// The array key is an integer, so the chosen option is a heading, not a real choice
		return '';
	} else if ( array_key_exists( $value, woocustomizer_library_get_font_choices() ) ) {
		return $value;
	} else {
		return '';
	}
...
if ( ! function_exists( 'woocustomizer_library_get_google_fonts' ) ) :
/**
 ...
 */
function woocustomizer_library_get_google_fonts() {
	return apply_filters( 'woocustomizer_library_get_google_fonts', array(
		'ABeeZee' => array(
			'label'    => 'ABeeZee',
			'variants' => array(
				'regular',
