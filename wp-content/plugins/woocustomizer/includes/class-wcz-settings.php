 ...
 * Settings class file.
 *
 * @package WooCustomizer/Settings
 */
...
class WooCustomizer_Settings {
	...
	private function wcz_menu_settings() {
		return apply_filters(
			...
			array(
				...
				'page_title'  => __( 'WooCustomizer', 'woocustomizer' ),
				'menu_title'  => __( 'WooCustomizer', 'woocustomizer' ),
			...
			$settings['wcz_general'] = array(
				...
				'fields'      => array(
					...
					array(
						...
						'label'       => __( 'Delete all plugin data when WooCustomizer is deleted', 'woocustomizer' ),
						'description' => '',
						'type'        => 'checkbox',
						'default'     => '',
					),
			...
			$settings['wcz_thank_you'] = array(
				...
				'fields'      => array(
					array(
						...
						'type'        => 'promo',
						'default'     => '',
						'top_txt' => __( 'WooCustomizer offers the ability to build your own custom Thank You pages for WooCommerce, using default WordPress or any page builder you like, and to redirect the customer to your new default Thank You page, redirect them to different pages depending on the products they\'ve just bought, or to pages depending how they purchased the products on your store.', 'woocustomizer' ),
					),
				),
			);
		...
		if ( wcz_fs()->can_use_premium_code() ) {
			...
			$settings['wcz_thank_you'] = array(
				...
				'fields'      => array(
					...
					array(
						'id'          => 'set_ctp_help',
						'label'       => __( 'See WooCustomizer Documentation', 'woocustomizer' ),
						'description' => '',
						'type'        => 'doclink',
						'placeholder'     => esc_url( 'https://woocustomizer.com/documentation/' ),
					)
				)
			);
	...
	public function settings_page() {
		...
		$html 	.= '<h2>' . __( 'WooCustomizer', 'woocustomizer' ) . '</h2>' . "\n";
		...
		if ( is_array( $this->settings ) && 1 < count( $this->settings ) ) {
			...
			foreach ( $this->settings as $section => $data ) {
				...
				$tabisnew = '';
				if ( 'thank' == strtolower( $tabnew ) ) // ADJUST FOR NEW TABS
		...
		$html             .= '<div class="wcz-customizer-links">' . "\n";
			$html             .= '<a href="' . esc_url( wcz_fs()->get_account_url() ) . '">' . esc_html( __( 'Account', 'woocustomizer' ) ) . '</a>' . "\n";
			// Only displayed in the free version
			if ( ! wcz_fs()->can_use_premium_code() ) {
				$html             .= '<a href="' . esc_url( wcz_fs()->get_upgrade_url() ) . '" class="wcz-upgrade">' . esc_html( __( 'Upgrade', 'woocustomizer' ) ) . '</a>' . "\n";
			}
			// Contact is only for Premium users
			if ( wcz_fs()->can_use_premium_code() ) {
				$html             .= '<a href="' . esc_url( wcz_fs()->contact_url() ) . '">' . esc_html( __( 'Contact Us', 'woocustomizer' ) ) . '</a>' . "\n";
			}
		$html             .= '</div>' . "\n";
		$html             .= '<p>' . esc_html( __( 'Click here to open the WordPress Customizer and navigate to the -> WooCustomizer panel.', 'woocustomizer' ) ) . '</p>' . "\n";
		$html             .= '<a href="' . esc_url( admin_url( 'customize.php' ) ) . '" class="wcz-customizer-btn">' . esc_html( __( 'Customize Your Settings', 'woocustomizer' ) ) . '</a>' . "\n";
		$html             .= '<p>' . esc_html( __( 'Vote for', 'woocustomizer' ) ) . ' <a href="' . esc_url( 'https://woocustomizer.com/go/features-vote-link/' ) . '" target="_blank">' . esc_html( __( 'features you want added to WooCustomizer Pro', 'woocustomizer' ) ) . '</a></p>' . "\n";
		$html             .= '</div>' . "\n";
		$html             .= '</div>' . "\n";
