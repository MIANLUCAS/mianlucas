...
class URL_Coupons implements Model_Interface
{
    ...
    public function implement_url_coupon()
    {
        ...
        // Coupon codes are just post titles. So we pass it through 'sanitize_title'
        $coupon_code = isset($wp_query->query['name']) ? urldecode(sanitize_title($wp_query->query['name'])) : '';

        // If coupon is not present, then we just redirect normally to the cart.
        if (!$coupon_code || !apply_filters('acfw_check_additional_coupon_endpoint_args', true)) {
        ...
        $coupon_args = apply_filters('acfw_extract_coupon_endpoint_args', array('code' => $coupon_code));
