...
class WPML_Support implements Model_Interface {
    ...
    */
    ...
    public function register_translatable_strings_for_coupons() {
        ...
        if ( ! $this->_is_wpml_admin_translation_pages() ) return;
        ...
        if ( ! is_array( $query->posts ) || empty( $query->posts ) ) return;
    ...
    public function convert_amount_to_user_selected_currency( $amount , $is_reverse = false ) {
        ...
        if ( ! $multi_currency ) return $amount;

        $user_currency = $multi_currency->get_client_currency();
        $site_currency = wcml_get_woocommerce_currency_option();

        if ( $site_currency === $user_currency ) return $amount;

        if ( $is_reverse )
            return $multi_currency->prices->convert_price_amount_by_currencies( $amount , $user_currency , $site_currency ); // convert from user to base.
        else
            return $multi_currency->prices->convert_price_amount_by_currencies( $amount , $site_currency , $user_currency ); // convert from base to user.
    }
    ...
    public function add_instructions_to_coupon_url_field( $fields ) {
        ...
        if ( ! $sitepress ) return $fields;

        $fields = array_map( function( $f ) use ( $sitepress ) {
            
            if ( $f[ 'args' ][ 'id' ] === Plugin_Constants::META_PREFIX . 'coupon_url' ) {
                ...
                foreach ( $active_languages as $key => $lang ) {
                    ...
                    if ( $default_language === $key ) continue;
    ...
    */
    ...
    public function run() {
        ...
        if ( ! $this->_helper_functions->is_plugin_active( 'woocommerce-multilingual/wpml-woocommerce.php' ) ) return;
