...
class Notices implements Model_Interface , Initializable_Interface , Activatable_Interface {
    ...
    */
    ...
    public function display_acfw_notices() {
        ...
        foreach ( $this->get_all_admin_notice_options() as $notice_key => $notice_option ) {
            ...
            if ( ! $notice_option || get_option( $notice_option ) !== 'yes' || $notice_key === 'review_request' ) continue;
            ...
            if ( ! $this->is_acfw_screen( $screen , $post_type ) ) continue;
    ...
    public function display_acfw_notices_on_settings() {
        ...
        foreach ( $this->get_all_admin_notice_options() as $notice_key => $notice_option ) {
            ...
            if ( ! $notice_option || get_option( $notice_option ) !== 'yes' || $notice_key === 'review_request' ) return;
    ...
    public function print_admin_notice_content( $notice_key , $notice_option , $on_settings = false ) {
        ...
        if ( isset( $notice_paths[ $notice_key ] ) ) include $notice_paths[ $notice_key ];
    }
    ...
    public function display_acfw_notice_in_settings( $settings , $current_section ) {
        ...
        if ( $current_section === 'acfw_premium' || get_option( Plugin_Constants::SHOW_UPGRADE_NOTICE ) !== 'yes' ) return $settings;
    ...
    public function enqueue_admin_notice_scripts( $screen , $post_type ) {
        ...
        if ( ! $this->is_acfw_screen( $screen , $post_type ) || ! current_user_can( 'manage_options' ) ) return;

        $is_enqueued = false;

        foreach ( $this->get_all_admin_notice_options() as $notice_key => $notice_option ) {

            if ( get_option( $notice_option ) !== 'yes' ) continue;
    ...
    */
    ...
    private function _schedule_single_notice_cron( $key , $option , $days ) {
        ...
        if ( wp_next_scheduled( Plugin_Constants::NOTICES_CRON , array( $key ) ) || get_option( $option , 'snooze' ) !== 'snooze' ) return;
    ...
    public function trigger_show_notice( $key ) {
        ...
        if ( ! isset( $notice[ 'option' ] ) || get_option( $notice[ 'option' ] ) === 'dismissed' ) return;
    ...
    public function reschedule_notice_cron( $key , $value ) {
        ...
        if ( $timestamp ) wp_unschedule_event( $timestamp , Plugin_Constants::NOTICES_CRON , array( $key ) );
    ...
    */
    ...
    public function ajax_dismiss_admin_notice() {
        ...
        if ( defined( 'DOING_AJAX' ) && DOING_AJAX && current_user_can( 'manage_options' ) ) {
            ...
            $response   = $response == 'snooze' ? 'snooze' : 'dismissed';
