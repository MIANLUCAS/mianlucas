...
class Admin_App implements Model_Interface, Initializable_Interface
{
    ...
    public function register_react_scripts($screen, $post_type)
    {
        ...
        $settings_app_dev = false;

        if (WP_DEBUG && defined('ACFW_SETTINGS_APP_URL') && ACFW_SETTINGS_APP_URL) {
            $site_data        = wp_remote_get(ACFW_SETTINGS_APP_URL);
            $settings_app_dev = !is_wp_error($site_data);
        }

        $handle = '';

        do_action('acfw_admin_app_enqueue_scripts_before', $screen, $post_type, $settings_app_dev);

        if ($settings_app_dev) {

            preg_match_all('/src=([\'"])?((?(1).+?|[^\s>]+))(?(1)\1)/', $site_data['body'], $matches);

            foreach ($matches[2] as $script) {
                $handle = Plugin_Constants::TOKEN . str_replace(array('/static/js/', '.js'), '', $script);
                wp_enqueue_script($handle, ACFW_SETTINGS_APP_URL . $script, array(), Plugin_Constants::VERSION, true);
            }

        } else {

            $app_js_path  = $this->_constants->JS_ROOT_PATH() . '/app/settings/build/static/js/';
            $app_css_path = $this->_constants->JS_ROOT_PATH() . '/app/settings/build/static/css/';
                ...
                if ($js_files = \scandir($app_js_path)) {
                    foreach ($js_files as $key => $js_file) {
                        if (strpos($js_file, '.js') !== false && strpos($js_file, '.js.map') === false && strpos($js_file, '.js.LICENSE.txt') === false) {
                            ...
                            wp_enqueue_script($handle, $this->_constants->JS_ROOT_URL() . 'app/settings/build/static/js/' . $js_file, array(), Plugin_Constants::VERSION, true);
                        }
                    }
                }
            }
                ...
                if ($css_files = \scandir($app_css_path)) {
                    foreach ($css_files as $key => $css_file) {
                        if (strpos($css_file, '.css') !== false && strpos($css_file, '.css.map') === false) {
                            wp_enqueue_style(Plugin_Constants::TOKEN . $key, $this->_constants->JS_ROOT_URL() . 'app/settings/build/static/css/' . $css_file, array(), Plugin_Constants::VERSION, 'all');
                        }
                    }
                }
            }
        ...
        do_action('acfw_admin_app_enqueue_scripts_after', $screen, $post_type, $settings_app_dev);
    }
