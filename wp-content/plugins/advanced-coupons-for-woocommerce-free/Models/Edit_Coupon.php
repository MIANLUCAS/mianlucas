...
class Edit_Coupon implements Model_Interface, Initializable_Interface, Activatable_Interface
{
    ...
    public function add_coupons_list_in_order_preview_popup($data, $order)
    {
        ...
        $markup       = '<div class="acfw-order-used-coupons" style="padding: 0 1.5em 1.5em;">';
        $markup .= __('<strong>Coupons</strong><br>', 'advanced-coupons-for-woocommerce-free');
        $temp = array();
        ...
        if (is_array($used_coupons) && !empty($used_coupons)) {
            ...
            foreach ($used_coupons as $coupon) {
                ...
                $edit_coupon = $coupon_id ? admin_url('post.php?post=' . $coupon_id . '&action=edit') : '';
                $temp[]      = $edit_coupon ? sprintf('<a class="used-coupon" href="%s">%s</a>', $edit_coupon, $coupon) : sprintf('<span class="used-coupon">%s</span>', $coupon);
            }

            $markup .= implode(', ', $temp);
        } else {
            $markup .= '<span class="no-coupons">' . __('No coupons used', 'advanced-coupons-for-woocommerce-free') . '</span>';
        }
    ...
    public function url_coupons_admin_data_panel($coupon_id)
    {
        ...
        $fields      = apply_filters('acfw_url_coupons_admin_data_panel_fields', array(
            array(
                ...
                'args' => array(
                    'id'          => Plugin_Constants::META_PREFIX . 'disable_url_coupon',
                    'label'       => __('Disable Coupon URL', 'advanced-coupons-for-woocommerce-free'),
                    'description' => __('When checked, it disables the coupon url functionality for the current coupon.', 'advanced-coupons-for-woocommerce-free'),
                    'value'       => $coupon->get_advanced_prop_edit('disable_url_coupon'),
                ),
            ),
            array(
                ...
                'args' => array(
                    ...
                    'description'       => $url_warning . __('<br>Visitors to this link will have the coupon code applied to their cart automatically.', 'advanced-coupons-for-woocommerce-free'),
                    'type'              => 'url',
                    'data_type'         => 'url',
    ...
    private function _get_special_symbols_warning_text($coupon)
    {
        ...
        if (!preg_match('/[\'!^£$%&*()}{@#~?><>,|=_+¬]/', $coupon->get_code()) || $coupon->get_advanced_prop('code_url_override')) {
            return '';
        }
    ...
    public function cart_conditions_admin_data_panel($post)
    {
        ...
        $cgroup_actions        = include $this->_constants->VIEWS_ROOT_PATH() . 'coupons' . DIRECTORY_SEPARATOR . 'view-cart-condition-group-actions.php';
    ...
    public function bogo_deals_admin_data_tab($coupon_data_tabs)
    {
        ...
        $coupon_data_tabs['acfw_bogo_deals'] = array(
            'label'  => __('BOGO Deals', 'advanced-coupons-for-woocommerce-free'),
            'target' => 'acfw_bogo_deals',
            'class'  => '',
        );
    ...
    public function bogo_deals_admin_data_panel($coupon_id)
    {
        ...
        $trigger_apply_type_descs = apply_filters('acfw_bogo_trigger_apply_type_descs', array(
            'specific-products' => __('Specific Product – good for straight forward deals where only one, non-variable product is involved', 'advanced-coupons-for-woocommerce-free'),
        ));

        $trigger_apply_type_options = apply_filters('acfw_bogo_trigger_apply_type_options', array(
            'specific-products' => __('Specific Products', 'advanced-coupons-for-woocommerce-free'),
        ));
    ...
    public function save_url_coupons_data($coupon_id)
    {
        ...
        if ($this->_helper_functions->is_module(Plugin_Constants::URL_COUPONS_MODULE)) {
            ...
            $disable_coupon_url = isset($_POST[Plugin_Constants::META_PREFIX . 'disable_url_coupon']) ? sanitize_text_field($_POST[Plugin_Constants::META_PREFIX . 'disable_url_coupon']) : '';
            ...
            $coupon->set_advanced_prop('disable_url_coupon', 'yes' == $disable_coupon_url ? 'yes' : '');
    ...
    public function prevent_multiple_spaces_in_coupon_code($data)
    {
        ...
        if ('shop_coupon' === $data['post_type'] && isset($_POST['post_title'])) {
            $data['post_title'] = trim(preg_replace('!\s+!', ' ', sanitize_text_field($_POST['post_title'])));
        }
...
foreach ($field['options'] as $key => $value) {
    ...
    public function register_clone_coupon_metabox($post_type, $post)
    {
        ...
        if ('shop_coupon' !== $post_type || !isset($_GET['action']) || get_post_status($post) !== 'publish') {
            return;
        }

        $metabox = function ($post) {
            ?>
            <a class="button" href="<?php echo wp_nonce_url('admin.php?action=acfw_clone_coupon&post=' . $post->ID, 'acfw_clone_coupon_nonce', 'nonce'); ?>">
                <?php _e('Clone Coupon', 'advanced-coupons-for-woocommerce-free');?>
            </a>
            <?php
};

        add_meta_box('acfw-clone-coupon', __('Clone Coupon', 'advanced-coupons-for-woocommerce-free'), $metabox, 'shop_coupon', 'side');
    }
    ...
    public function run()
    {
        ...
        if ($this->_helper_functions->is_module(Plugin_Constants::BOGO_DEALS_MODULE)) {
            ...
            add_filter('woocommerce_coupon_data_tabs', array($this, 'bogo_deals_admin_data_tab'), 20, 1);
            add_action('woocommerce_coupon_data_panels', array($this, 'bogo_deals_admin_data_panel'));
        }
        ...
        // Clone coupon
        add_action('add_meta_boxes', array($this, 'register_clone_coupon_metabox'), 20, 2);
