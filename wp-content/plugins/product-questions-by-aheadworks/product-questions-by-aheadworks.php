<?php
/**
 * Plugin Name: Product Questions By Aheadworks
 * Plugin URI: https://www.aheadworks.com/
 * Description: Product Questions plugin for WooCommerce. Adds Question & Answer area to product pages.
 * Author: Aheadworks
	...
	public static function aw_pq_product_questions_installer() {
		if (is_admin()) {
			if (!in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins')))) {
				...
				add_action('admin_notices', array('AwPqProductQuestionsAdmin','aw_pq_self_deactivate_notice'));
...
View it on the site:')", "{$admin_emails}"));
	...
	public static function aw_pq_product_questions_menu() {
		add_menu_page('Product Questions', 'Product Questions', 'edit_posts', 'product-questions', array('AwPqProductQuestionsAdmin','aw_pq_product_question_setting') , plugin_dir_url(__FILE__) . '/admin/images/questions.png', 75);
