<?php
/*
Class Name: VI_WOOCOMMERCE_THANK_YOU_PAGE_Frontend_Account
...
class VI_WOOCOMMERCE_THANK_YOU_PAGE_Frontend_Account {
	protected $settings;
	...
	public function woocommerce_my_account_my_orders_column_wtypc_coupon( $order ) {
		...
		if ( $coupon_code ) {
			...
			if ( $coupon ) {
				...
				$title        .= sprintf( __( 'Used: %s/%s.', 'woocommerce-thank-you-page-customizer' ), $usage_count, $usage_limit, 'woocommerce-thank-you-page-customizer' );
	...
	public function woocommerce_account_orders_columns( $columns ) {
		if ( isset( $columns['order-actions'] ) ) {
			...
			$columns['wtypc_coupon']  = __( 'Coupon gift', 'woocommerce-thank-you-page-customizer' );
