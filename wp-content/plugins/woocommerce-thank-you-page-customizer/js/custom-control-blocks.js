jQuery(document).ready(function ($) {
    ...
    function WtypSortable() {
        $('.woocommerce-thank-you-page-container__block').sortable({
            ...
            start: function (event, ui) {
                ...
                start_p = $('.woocommerce-thank-you-page-container__block .woocommerce-thank-you-page-products').index(ui.item);
            },
            ...
            stop: function (event, ui) {
                ...
                let position_p = $('.woocommerce-thank-you-page-container__block .woocommerce-thank-you-page-products').index(ui.item);
                WtypApplyChange(position, position_p);
            }
        }).disableSelection();
    }
    ...
    $('body').on('click', '.woocommerce-thank-you-page-container__block .woocommerce-thank-you-page-remove', function (event) {
        ...
        let position_p = $('.woocommerce-thank-you-page-products').index($(this).parent());
        if ($(this).parent().data()['block_item'] === 'text_editor' || $(this).parent().data()['block_item'] === 'products') {
            $(this).parent().remove();
        } else {
    ...
    function WtypApplyChange(position, position_p) {
        let row = [],
        ...
        let products = JSON.parse(wp.customize('woo_thank_you_page_params[products]').get());
        let blocks_old = JSON.parse(wp.customize('woo_thank_you_page_params[blocks]').get());
        if (position_p >= 0) {
            /*before*/
            /*after*/
            let textEditorVal = $('.woocommerce-thank-you-page-container .woocommerce-thank-you-page-products');

            if (products.length > textEditorVal.length) {
                products.splice(position_p, 1);
            } else if (products.length < textEditorVal.length) {
                products.splice(position_p, 0, '');
            } else {
                if (start_p != position_p) {
                    let textStart = products[start_p];
                    let textStop = products[position_p];
                    if (move_p === true) {
                        if (start_p > position_p) {
                            products.splice(start_p, 1);
                            products.splice(position_p, 0, textStart);
                        } else if (start_p < position_p) {
                            products.splice(start_p, 1);
                            products.splice(position_p, 0, textStart);
                        }
                        ...
                        if (start_p > position_p) {
                            ...
                            if (start_p + 1 == position_p) {
                                products.splice(start_p, 1, textStop);
                                products.splice(position_p, 1, textStart);
                            } else {
                                products.splice(start_p, 1);
                                products.splice(position_p, 0, textStart);
                            }
        ...
        wp.customize('woo_thank_you_page_params[products]').set(JSON.stringify(products));
    ...
    function AddItemToBlock() {
        $('.woocommerce-thank-you-page-components').on('click', '.woocommerce-thank-you-page-item', function () {
            ...
            for (let i = 0; i <= current_block; i++) {
                ...
                position_p += blocks.eq(i).find('.woocommerce-thank-you-page-products').length;
            }
            //set text editor before set blocks
            let textEditor = JSON.parse(wp.customize('woo_thank_you_page_params[text_editor]').get());
            ...
            } else if ($(this).data()['block_item'] === 'products') {
                products.splice(position_p, 0, {
                    columns: '4',
